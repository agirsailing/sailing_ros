...
def choose_navigation_direction(gps, compass, now, max_age, speed_threshold,
    ...
    if gps is not None:
        ...
        if (0 <= now - stamp <= max_age and valid
            ...
            return course % 360.0, 'gps', stamp
    if compass is not None:
        stamp, x, y = compass
        if 0 <= now - stamp <= max_age and math.isfinite(x) and math.isfinite(y):
            x, y = x_sign * (x - x_bias), y_sign * (y - y_bias)
            if x != 0 or y != 0:
                return (math.degrees(math.atan2(y, x)) + offset_deg) % 360.0, 'compass', stamp
    return float('nan'), 'none', None

def choose_heading(gps, compass, now, max_age, speed_threshold,
                   x_sign, y_sign, x_bias, y_bias, offset_deg):
    """Scalar interface for consumers that do not need source metadata."""
    return choose_navigation_direction(
        gps, compass, now, max_age, speed_threshold,
        x_sign, y_sign, x_bias, y_bias, offset_deg)[0]
